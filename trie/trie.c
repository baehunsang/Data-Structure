...
/*
	Initial function, insert one character in the trie.
	Return the pointer of the next processing node.
*/
TRIE* _insert(TRIE* root, char ch);

/*
	Initial function, if the input string has the upper character, then change that character to lower character.
*/
char* _changeStr(char* str);
...
int trieInsert(TRIE* root, char* str, int dic_index) {
	char* inputStr = _changeStr(str);
	TRIE* proceNode = root;
	for (int i = 0; i < strlen(inputStr); i++) {
		if ((inputStr[i] >= 'a' && inputStr[i] <= 'z') || inputStr[i] == EOW) {
			proceNode = _insert(proceNode, inputStr[i]);
			if (!proceNode) { // Overflow is occured.
				return 0;
			}
		}
		else {
			return 0; // the string is wrong.
		}
	}
	if (proceNode->index == -1) {
		proceNode->index = dic_index;
		return 1;
	}
	return 0; // Already the string exists.
}

TRIE* _insert(TRIE* root, char ch) {
	if (root->subtrees[getIndex(ch)] == NULL) {
		root->subtrees[getIndex(ch)] = trieCreateNode();
		return root->subtrees[getIndex(ch)];
	}
	else {
		return root->subtrees[getIndex(ch)];
	}
...
int trieSearch(TRIE* root, char* str) {
	TRIE* proceNode = root;
	for (int i = 0; i < strlen(str); i++) {
		if ((str[i] >= 'a' && str[i] <= 'z') || str[i] == EOW) {
			proceNode = proceNode->subtrees[getIndex(str[i])];
			if (!proceNode) {
				return -1; //there are not the string that start with 'str[0]~str[i]' 
			}
		}
		else{
		
			return -1; //the input string is wrong.
		}
	}
	return proceNode->index;
}
...
void triePrefixList(TRIE* root, char* str, char* dic[]) {
	...
	for (int i = 0; i < strlen(str); i++) {
		if ((str[i] >= 'a' && str[i] <= 'z') || str[i] == EOW) {
			proceNode = proceNode->subtrees[getIndex(str[i])];
			if (!proceNode) { // If there are not the string that start with 'str' then this function is not working.
				return;
			}
		}
		else {
