...
int binary_search(const void* key, const void* base, size_t nmemb, size_t size, int (*compare)(const void*, const void*)) {
	...
	while (start <= last) {
		mid = (start + last) / 2;
		tName *objNum = (tName*)base + mid;
		if (compare(key, objNum)==0) {
			return mid;
		}
		else if (compare(key, objNum) > 0) {
			start = mid + 1;
			continue;
		}
		else{
...
void load_names(FILE* fp, int year_index, tNames* names) {
	char buffer[100];
	char* strPtr;
	tName* nameBuf = (tName*)malloc(sizeof(tName));     //읽어들인 파일 내용을 일시적으로 저장
	memset(nameBuf, 0, sizeof(tName));
	int index;
	if (year_index == 0) { //처음
		char buffer[100];
	...
	while (!feof(fp)) {
		...
		strcpy(nameBuf->name , strtok(buffer, ","));
		nameBuf->sex = *(strtok(NULL, ","));
		nameBuf->freq[year_index] = atoi(strtok(NULL, ","));
		index = binary_search(nameBuf, names->data, names->len,sizeof(tName), compare);

		if (index == names->len) {
			memset(&(names->data[index]), 0, sizeof(tName));
			strcpy((names->data[index]).name, nameBuf->name);
			(names->data[index]).sex = nameBuf->sex;
			(names->data[index]).freq[year_index] = nameBuf->freq[year_index];
			names->len++;
		}                                //맨 끝에 삽입하는 경우
		else {
			if (!strncmp(nameBuf->name, (names->data)[index].name, 20) && ((nameBuf->sex) == (names->data)[index].sex)) {
				(names->data[index]).freq[year_index] = nameBuf->freq[year_index];
				continue;
			}                          //index값이 중간 or 맨 앞이면서 key와 기존 정보의 이름, 성별이 동일
			else {
				memmove(&(names->data[index]) + 1, &(names->data[index]), sizeof(tName)*(((names->len - 1) - index) + 1));
				memset(&(names->data[index]), 0, sizeof(tName));
				strcpy((names->data[index]).name, nameBuf->name);
				(names->data[index]).sex = nameBuf->sex;
				(names->data[index]).freq[year_index] = nameBuf->freq[year_index];
				names->len++;
			}                     //index 값이 중간 or 맨 앞이면서 중간에 key값을 삽입해야 하는 경우
		}
	}
	free(nameBuf);             //임시 저장공간 해제
}
